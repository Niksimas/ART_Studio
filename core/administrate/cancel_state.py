...
async def check_state(message: Message, state: FSMContext):
    ...
    await message.answer(f"Бот находится в ожидании ответа. Посмотрите историю сообщений для дальнейшего ответа или нажмите кнопку отмены ожидания ниже",
...
async def cancel_state(callback: CallbackQuery, state: FSMContext):
    ...
    if current_state is None:
        return
    await state.clear()
    await callback.message.edit_text("Ожидание ответа успешно отменено.")
